            ...
            switch (current_entity.type) {
                ...
                case carnivore:
                    ...
                    if (random_action(CARNIVORE_MOVE_PROBABILITY)) {
                        ...
                        // Escolha aleatoriamente uma direção de movimento (incluindo células com herbívoros)
                        int random_direction = random_integer(0, 3);
                        int new_i = i;
                        int new_j = j;
                        ...
                        // Verifique se a célula de destino está ocupada por um herbívoro
                        if (entity_grid[new_i][new_j].type == herbivore) {
                            // O carnívoro move-se para a célula de destino e consome o herbívoro
                            entity_grid[new_i][new_j] = entity_grid[i][j];
                            ...
                            entity_grid[new_i][new_j].energy += 20;

                            // Reduza a energia do carnívoro devido ao movimento
                            entity_grid[new_i][new_j].energy -= 5;
                        } else {
                            // A célula de destino está vazia, o carnívoro apenas se move
                            entity_grid[new_i][new_j] = entity_grid[i][j];
                            entity_grid[i][j].type = empty;
                            entity_grid[i][j].energy = 0;
                            entity_grid[i][j].age = 0;
                    ...
                    if (random_action(CARNIVORE_EAT_PROBABILITY)) {
                        // Carnívoro tenta se alimentar
                        // Implemente a lógica de alimentação aqui

                        // Verifique as células adjacentes para encontrar um herbívoro
                        for (int direction = 0; direction < 4; ++direction) {
                            ...
                            // Verifique se a célula adjacente contém um herbívoro
                            if (entity_grid[new_i][new_j].type == herbivore) {
                                // O carnívoro come o herbívoro
                                entity_grid[new_i][new_j].type = empty;
                                entity_grid[new_i][new_j].energy = 0;
                                entity_grid[new_i][new_j].age = 0;

                                // Ganhe 20 unidades de energia ao comer um herbívoro
                                entity_grid[i][j].energy += 20;

                                // Saia do loop, pois o carnívoro já se alimentou
                                break;
                            }
                        }
                    }
                    if (random_action(CARNIVORE_REPRODUCTION_PROBABILITY)) {
                        // Carnívoro tenta se reproduzir
                        // Implemente a lógica de reprodução aqui

                        // Verifique se a energia do carnívoro é suficiente para reprodução
                        if (entity_grid[i][j].energy > 20) {
                            // Procure uma célula adjacente vazia para colocar a prole
                            for (int direction = 0; direction < 4; ++direction) {
                                int new_i = i;
                                int new_j = j;
                                ...
                                // Verifique se a célula adjacente está vazia
                                if (entity_grid[new_i][new_j].type == empty) {
                                    // Crie uma nova prole (carnívoro) na célula adjacente vazia
                                    ...
                                    // Reduza a energia do carnívoro pai após a reprodução
                                    entity_grid[i][j].energy -= 10;

                                    // Saia do loop, pois o carnívoro já se reproduziu
                                    break;
                                }
                            }
                        }
